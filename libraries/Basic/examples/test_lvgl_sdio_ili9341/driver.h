...
/* Упрощенная версия SDIO->SPI драйвера предоставленного компанией WinnerMicro */

// Для использования альтернативных пинов смотри даташит и SDK. Там все необходимое есть
...
void sdio_init() {
    ...
    // Настройка скорости шины. На разных дисплеях по разному
...
void write_sdio_spi_dma(u32* data, u32 len) 
{
    ...
    while(len>0){
        ...
        offset+=datalen/4;
        while(1){
            if ((SDIO_HOST->BUF_CTL & 0x400) == 0x00) // Ожидание освобождения буфера
                break;
        }
