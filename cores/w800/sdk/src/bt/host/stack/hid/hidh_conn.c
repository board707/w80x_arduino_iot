...
**
*******************************************************************************/
void hidh_sec_check_complete_term(BD_ADDR bd_addr, tBT_TRANSPORT transport, void *p_ref_data,
                                  uint8_t res)
{
    ...
    if(res == BTM_SUCCESS && p_dev->conn.conn_state == HID_CONN_STATE_SECURITY) {
        p_dev->conn.disc_reason =
                        HID_SUCCESS;  /* Authentication passed. Reset disc_reason (from HID_ERR_AUTH_FAILED) */
        p_dev->conn.conn_state = HID_CONN_STATE_CONNECTING_INTR;
        /* Send response to the L2CAP layer. */
        L2CA_ConnectRsp(p_dev->addr, p_dev->conn.ctrl_id, p_dev->conn.ctrl_cid, L2CAP_CONN_OK,
                        L2CAP_CONN_OK);
...
**
*******************************************************************************/
static void hidh_l2cif_connect_ind(BD_ADDR  bd_addr, uint16_t l2cap_cid, uint16_t psm,
                                   uint8_t l2cap_id)
{
    tHID_CONN    *p_hcon;
    ...
    if(psm == HID_PSM_CONTROL) {
        ...
        p_hcon->disc_reason =
                        HID_L2CAP_CONN_FAIL;  /* In case disconnection occurs before security is completed, then set CLOSE_EVT reason code to 'connection failure' */
...
**
*******************************************************************************/
void hidh_sec_check_complete_orig(BD_ADDR bd_addr, tBT_TRANSPORT transport, void *p_ref_data,
                                  uint8_t res)
{
    tHID_HOST_DEV_CTB *p_dev = (tHID_HOST_DEV_CTB *) p_ref_data;
    uint8_t dhandle;
...
static void hidh_l2cif_config_ind(uint16_t l2cap_cid, tL2CAP_CFG_INFO *p_cfg)
{
    ...
    if(l2cap_cid == p_hcon->ctrl_cid) {
        ...
        if((p_hcon->conn_flags & HID_CONN_FLAGS_IS_ORIG) &&
            ...
            p_hcon->disc_reason =
                            HID_L2CAP_CONN_FAIL;  /* Reset initial reason for CLOSE_EVT: Connection Attempt was made but failed */
...
static void hidh_l2cif_cong_ind(uint16_t l2cap_cid, uint8_t congested)
{
    ...
    HIDH_TRACE_EVENT("HID-Host Rcvd L2CAP congestion status, CID: 0x%x  Cong: %d", l2cap_cid,
                     congested);
