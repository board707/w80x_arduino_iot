...
typedef struct {
    ...
    uint8_t
    appl_knows_rem_name;    /* set by application if it knows the remote name of the peer device.
...
typedef struct {
    ...
    uint8_t
    *p_features; /* pointer to the remote device's features page[0] (supported features page) */
#if BLE_INCLUDED == TRUE
