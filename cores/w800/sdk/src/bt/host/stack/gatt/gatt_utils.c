...
**
*******************************************************************************/
tGATTS_PENDING_NEW_SRV_START *gatt_sr_is_new_srv_chg(tBT_UUID *p_app_uuid128, tBT_UUID *p_svc_uuid,
        uint16_t svc_inst)
{
...
**
*******************************************************************************/
uint8_t gatt_is_last_attribute(tGATT_SRV_LIST_INFO *p_list, tGATT_SRV_LIST_ELEM *p_start,
                               tBT_UUID value)
{
...
**
*******************************************************************************/
uint8_t gatt_sr_find_i_rcb_by_app_id(tBT_UUID *p_app_uuid128, tBT_UUID *p_svc_uuid,
                                     uint16_t svc_inst)
{
    uint8_t           i_rcb = 0;
    tGATT_SR_REG    *p_sreg;
...
**
*******************************************************************************/
void gatt_sr_get_sec_info(BD_ADDR rem_bda, tBT_TRANSPORT transport, uint8_t *p_sec_flag,
                          uint8_t *p_key_size)
{
    uint8_t           sec_flag = 0;
...
{
    GATT_TRACE_DEBUG("gatt_set_err_rsp enable=%d op_code=%d, err_status=%d", enable, req_op_code,
                     err_status);
...
**
*******************************************************************************/
void gatt_sr_update_cback_cnt(tGATT_TCB *p_tcb, tGATT_IF gatt_if, uint8_t is_inc,
...
**
*******************************************************************************/
void gatt_sr_update_prep_cnt(tGATT_TCB *p_tcb, tGATT_IF gatt_if, uint8_t is_inc,
                             uint8_t is_reset_first)
{
    uint8_t idx = ((uint8_t) gatt_if) - 1 ;
...
**
*******************************************************************************/
uint8_t gatt_find_app_hold_link(tGATT_TCB *p_tcb, uint8_t start_idx, uint8_t *p_found_idx,
                                tGATT_IF *p_gatt_if)
{
    uint8_t i;
    uint8_t found = FALSE;
...
**
*******************************************************************************/
uint8_t gatt_cmd_enq(tGATT_TCB *p_tcb, uint16_t clcb_idx, uint8_t to_send, uint8_t op_code,
                     BT_HDR *p_buf)
{
    ...
    if((p_tcb = gatt_find_tcb_by_addr(bda, transport)) != NULL) {
        ...
        for(i = 0; i < GATT_MAX_APPS; i ++) {
            ...
            if(p_reg->in_use && p_reg->app_cb.p_conn_cb) {
                ...
                GATT_TRACE_DEBUG("found p_reg tcb_idx=%d gatt_if=%d  conn_id=0x%x", p_tcb->tcb_idx, p_reg->gatt_if,
                                 conn_id);
...
**
*******************************************************************************/
uint8_t gatt_update_auto_connect_dev(tGATT_IF gatt_if, uint8_t add, BD_ADDR bd_addr,
                                     uint8_t is_initator)
{
    uint8_t         ret = FALSE;
    tGATT_REG        *p_reg;
