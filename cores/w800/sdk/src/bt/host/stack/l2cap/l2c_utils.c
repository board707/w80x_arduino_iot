...
tL2C_LCB *l2cu_allocate_lcb(BD_ADDR p_bd_addr, uint8_t is_bonding, tBT_TRANSPORT transport)
{
    ...
    for(xx = 0; xx < MAX_L2CAP_LINKS; xx++, p_lcb++) {
        if(!p_lcb->in_use) {
            ...
            p_lcb->tx_data_len     =
                            btm_cb.devcb.ble_packet_len;//controller_get_interface()->get_ble_default_data_packet_length();
                               ...
                               uint16_t p1, uint16_t p2)
{
    ...
    if((p_buf = l2cu_build_header(p_lcb, (uint16_t)(L2CAP_CMD_REJECT_LEN + param_len), L2CAP_CMD_REJECT,
                                  rem_id)) == NULL) {
...
void l2cu_send_peer_connect_rsp(tL2C_CCB *p_ccb, uint16_t result, uint16_t status)
{
    ...
    if((p_buf = l2cu_build_header(p_ccb->p_lcb, L2CAP_CONN_RSP_LEN, L2CAP_CMD_CONN_RSP,
                                  p_ccb->remote_id)) == NULL) {
...
void l2cu_send_peer_disc_req(tL2C_CCB *p_ccb)
{
    ...
    if((p_buf = l2cu_build_header(p_ccb->p_lcb, L2CAP_DISC_REQ_LEN, L2CAP_CMD_DISC_REQ,
                                  p_ccb->local_id)) == NULL) {
...
void l2cu_send_peer_echo_req(tL2C_LCB *p_lcb, uint8_t *p_data, uint16_t data_len)
{
    ...
    if((p_buf = l2cu_build_header(p_lcb, (uint16_t)(L2CAP_ECHO_REQ_LEN + data_len), L2CAP_CMD_ECHO_REQ,
                                  p_lcb->id)) == NULL) {
...
void l2cu_send_peer_echo_rsp(tL2C_LCB *p_lcb, uint8_t id, uint8_t *p_data, uint16_t data_len)
{
    ...
    if((p_buf = l2cu_build_header(p_lcb, (uint16_t)(L2CAP_ECHO_RSP_LEN + data_len), L2CAP_CMD_ECHO_RSP,
                                  id)) == NULL) {
...
void l2cu_send_peer_info_req(tL2C_LCB *p_lcb, uint16_t info_type)
{
    ...
    btu_start_timer(&p_lcb->info_resp_timer, BTU_TTYPE_L2CAP_INFO,
                    L2CAP_WAIT_INFO_RSP_TIMEOUT_MS / 1000);
#endif
    l2c_link_check_send_pkts(p_lcb, NULL, p_buf);
}
...
tL2C_CCB *l2cu_allocate_ccb(tL2C_LCB *p_lcb, uint16_t cid)
{
    ...
    p_ccb->our_cfg.flush_to              = p_ccb->peer_cfg.flush_to              =
            ...
            if(HCI_SWITCH_SUPPORTED(BTM_ReadLocalFeatures())) {
                ...
                if(BTM_SwitchRole(p_lcb_cur->remote_bd_addr, HCI_ROLE_MASTER, NULL) == BTM_CMD_STARTED) {
                    ...
                    btu_start_timer(&p_lcb->l2c_lcb_timer, BTU_TTYPE_L2CAP_LINK,
                                    L2CAP_LINK_ROLE_SWITCH_TIMEOUT_MS / 1000);
#endif
                    return (TRUE);
                }
            }
...
uint8_t l2cu_set_acl_priority(BD_ADDR bd_addr, uint8_t priority, uint8_t reset_after_rs)
{
    ...
    if(BTM_IS_BRCM_CONTROLLER()) {
        ...
        if((!reset_after_rs && (priority != p_lcb->acl_priority)) ||
            ...
            BTM_VendorSpecificCommand(HCI_BRCM_SET_ACL_PRIORITY, HCI_BRCM_ACL_PRIORITY_PARAM_SIZE, command,
                                      NULL);
...
void l2cu_adjust_out_mps(tL2C_CCB *p_ccb)
{
    ...
    if(packet_size <= (L2CAP_PKT_OVERHEAD + L2CAP_FCR_OVERHEAD + L2CAP_SDU_LEN_OVERHEAD +
                       L2CAP_FCS_LEN)) {
        /* something is very wrong */
        L2CAP_TRACE_ERROR("l2cu_adjust_out_mps bad packet size: %u  will use MPS: %u", packet_size,
