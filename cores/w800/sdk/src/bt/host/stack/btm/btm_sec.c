...
#include "../../stack/include/l2c_int.h"
...
static tBTM_STATUS btm_sec_send_hci_disconnect(tBTM_SEC_DEV_REC *p_dev_rec, uint8_t reason,
...
static uint8_t  btm_sec_set_security_level(CONNECTION_TYPE conn_type, char *p_name,
        uint8_t service_id,
...
{
    BTM_TRACE_API("BTM_SetPairableMode()  allow_pairing: %u   connect_only_paired: %u", allow_pairing,
                  connect_only_paired);
        ...
        uint32_t mx_chan_id)
{
    ...
    if(is_originator) {
        ...
        sec_level &= ~(BTM_SEC_OUT_AUTHORIZE | BTM_SEC_OUT_ENCRYPT | BTM_SEC_OUT_AUTHENTICATE |
                       BTM_SEC_OUT_MITM);
...
**
*******************************************************************************/
void BTM_PINCodeReply(BD_ADDR bd_addr, uint8_t res, uint8_t pin_len, uint8_t *p_pin,
                                      ...
                                      uint8_t pin_len, uint8_t *p_pin, uint32_t trusted_mask[])
{
    ...
    BTM_TRACE_EVENT("BTM_SecBond: Remote sm4: 0x%x  HCI Handle: 0x%04x", p_dev_rec->sm4,
                                     ...
                                     void *p_ref_data)
{
    ...
    uint8_t is_originator;
    tBT_TRANSPORT transport =
                    BT_TRANSPORT_BR_EDR; /* should check PSM range in LE connection oriented L2CAP connection */
#if (L2CAP_UCD_INCLUDED == TRUE)
        ...
        if(is_originator) {
            if(((security_required & BTM_SEC_OUT_FLAGS) == 0) ||
                    ((((security_required & BTM_SEC_OUT_FLAGS) == BTM_SEC_OUT_AUTHENTICATE)
                      && (p_dev_rec->sec_flags & BTM_SEC_AUTHENTICATED))) ||
                    ((((security_required & BTM_SEC_OUT_FLAGS) == (BTM_SEC_OUT_AUTHENTICATE | BTM_SEC_OUT_ENCRYPT))
                      && (p_dev_rec->sec_flags & BTM_SEC_ENCRYPTED))) ||
            ...
            if(((security_required & BTM_SEC_IN_FLAGS) == 0) ||
                    ((((security_required & BTM_SEC_IN_FLAGS) == BTM_SEC_IN_AUTHENTICATE)
                      && (p_dev_rec->sec_flags & BTM_SEC_AUTHENTICATED))) ||
                    ((((security_required & BTM_SEC_IN_FLAGS) == (BTM_SEC_IN_AUTHENTICATE | BTM_SEC_IN_ENCRYPT))
                      && (p_dev_rec->sec_flags & BTM_SEC_ENCRYPTED))) ||
    ...
    if((!is_originator) && (security_required & BTM_SEC_MODE4_LEVEL4)) {
        ...
        uint8_t local_supports_sc = HCI_SC_CTRLR_SUPPORTED(
        ...
        if((btm_cb.security_mode == BTM_SEC_MODE_UNDEFINED ||
            ...
            if(is_originator) {
                if(((security_required & BTM_SEC_OUT_FLAGS) == 0) ||
                        ...
                        ((((security_required & BTM_SEC_OUT_FLAGS) == BTM_SEC_OUT_FLAGS) && btm_dev_authorized(p_dev_rec)
                          && btm_dev_encrypted(p_dev_rec)))) {
                    rc = BTM_SUCCESS;
                }
            } else {
                if(((security_required & BTM_SEC_IN_FLAGS) == 0) ||
                        ...
                        (((security_required & BTM_SEC_IN_FLAGS) == (BTM_SEC_IN_AUTHENTICATE | BTM_SEC_IN_ENCRYPT))
                         && btm_dev_encrypted(p_dev_rec)) ||
                        (((security_required & BTM_SEC_IN_FLAGS) == BTM_SEC_IN_AUTHORIZE) && (btm_dev_authorized(p_dev_rec)
                                || btm_serv_trusted(p_dev_rec, p_serv_rec))) ||
                        (((security_required & BTM_SEC_IN_FLAGS) == (BTM_SEC_IN_AUTHENTICATE | BTM_SEC_IN_AUTHORIZE))
                         && ((btm_dev_authorized(p_dev_rec) || btm_serv_trusted(p_dev_rec, p_serv_rec))
                             && btm_dev_authenticated(p_dev_rec))) ||
                        (((security_required & BTM_SEC_IN_FLAGS) == (BTM_SEC_IN_ENCRYPT | BTM_SEC_IN_AUTHORIZE))
                         && ((btm_dev_authorized(p_dev_rec) || btm_serv_trusted(p_dev_rec, p_serv_rec))
                             && btm_dev_encrypted(p_dev_rec))) ||
                        (((security_required & BTM_SEC_IN_FLAGS) == BTM_SEC_IN_FLAGS)  && btm_dev_encrypted(p_dev_rec)
                         && (btm_dev_authorized(p_dev_rec) || btm_serv_trusted(p_dev_rec, p_serv_rec)))) {
                    // Check for 16 digits (or MITM)
                    if(((security_required & BTM_SEC_IN_MIN_16_DIGIT_PIN) == 0) ||
                            (((security_required & BTM_SEC_IN_MIN_16_DIGIT_PIN) == BTM_SEC_IN_MIN_16_DIGIT_PIN)
        ...
        if((btm_cb.security_mode == BTM_SEC_MODE_UNDEFINED ||
            ...
            if(is_originator) {
                ...
                if(((security_required & BTM_SEC_IN_FLAGS) == 0) ||
                        ...
                        (((security_required & BTM_SEC_IN_FLAGS) == BTM_SEC_IN_AUTHORIZE) && (btm_dev_authorized(p_dev_rec)
                                || btm_serv_trusted(p_dev_rec, p_serv_rec))) ||
                        (((security_required & BTM_SEC_IN_FLAGS) == (BTM_SEC_IN_AUTHORIZE | BTM_SEC_IN_AUTHENTICATE))
                         && ((btm_dev_authorized(p_dev_rec) || btm_serv_trusted(p_dev_rec, p_serv_rec))
                             && btm_dev_authenticated(p_dev_rec))) ||
                        (((security_required & BTM_SEC_IN_FLAGS) == (BTM_SEC_IN_AUTHORIZE | BTM_SEC_IN_ENCRYPT))
                         && ((btm_dev_authorized(p_dev_rec) || btm_serv_trusted(p_dev_rec, p_serv_rec))
                             && btm_dev_encrypted(p_dev_rec))) ||
                        ((((security_required & BTM_SEC_IN_FLAGS) == (BTM_SEC_IN_AUTHENTICATE | BTM_SEC_IN_ENCRYPT))
                          && btm_dev_encrypted(p_dev_rec)))
    ...
    if((btm_cb.pairing_state == BTM_PAIR_STATE_WAIT_LOCAL_PIN) && p_bd_addr
        ...
        if((btm_cb.pairing_flags & BTM_PAIR_FLAGS_PIN_REQD) == 0 && btm_cb.api.p_pin_callback) {
            ...
            (*btm_cb.api.p_pin_callback)(p_dev_rec->bd_addr, p_dev_rec->dev_class, p_bd_name,
                                         ...
                                         : (uint8_t)(p_dev_rec->p_cur_service->security_flags & BTM_SEC_IN_MIN_16_DIGIT_PIN));
        }
...
void btm_sec_rmt_host_support_feat_evt(uint8_t *p)
{
    ...
    if(BTM_SEC_IS_SM4_UNKNOWN(p_dev_rec->sm4)) {
        ...
        BTM_TRACE_EVENT("btm_sec_rmt_host_support_feat_evt sm4: 0x%x features[0]: 0x%x", p_dev_rec->sm4,
                        features[0]);
    }
}
...
void btm_simple_pair_complete(uint8_t *p)
{
    ...
    if((p_dev_rec = btm_find_dev(evt_data.bd_addr)) == NULL) {
        BTM_TRACE_ERROR("btm_simple_pair_complete() with unknown BDA: %08x%04x",
                        (evt_data.bd_addr[0] << 24) + (evt_data.bd_addr[1] << 16) + (evt_data.bd_addr[2] << 8) +
                        evt_data.bd_addr[3],
...
void btm_rem_oob_req(uint8_t *p)
{
    ...
    if((NULL != (p_dev_rec = btm_find_dev(p_bda))) &&
        ...
        if((*btm_cb.api.p_sp_callback)(BTM_SP_RMT_OOB_EVT,
                                       (tBTM_SP_EVT_DATA *)&evt_data) == BTM_NOT_AUTHORIZED) {
    ...
    */
    ...
    if(p_dev_rec) {
        BTM_TRACE_EVENT("Security Manager: auth_complete PairState: %s  handle:%u  status:%d  dev->sec_state: %u  Bda:%08x, RName:%s",
                        ...
                        handle, status,
                        p_dev_rec->sec_state,
                        (p_dev_rec->bd_addr[2] << 24) + (p_dev_rec->bd_addr[3] << 16) + (p_dev_rec->bd_addr[4] << 8) +
                        p_dev_rec->bd_addr[5],
                        p_dev_rec->sec_bd_name);
    } else {
    ...
    if(status != HCI_SUCCESS) {
        if((old_sm4 & BTM_SM4_RETRY) == 0) {
             ...
             * Legacy device do not need this. the controller will drive the retry automatically */
                ...
                p_dev_rec->sm4 |= BTM_SM4_RETRY;
                p_dev_rec->sec_flags &= ~BTM_SEC_LINK_KEY_KNOWN;
                BTM_TRACE_DEBUG("Retry for missing key sm4:x%x sec_flags:0x%x", p_dev_rec->sm4,
                                p_dev_rec->sec_flags);
...
void btm_sec_connected(uint8_t *bda, uint16_t handle, uint8_t status, uint8_t enc_mode)
{
    ...
    */
    ...
    btm_acl_created(bda, p_dev_rec->dev_class, p_dev_rec->sec_bd_name, handle, HCI_ROLE_SLAVE,
                    BT_TRANSPORT_BR_EDR);
...
void btm_sec_disconnected(uint16_t handle, uint8_t reason)
{
    ...
    BTM_TRACE_EVENT("%s sec_req:x%x state:%s reason:%d bd_addr:%02x:%02x:%02x:%02x:%02x:%02x"
                    "  remote_name:%s", __func__, p_dev_rec->security_required,
                    btm_pair_state_descr(btm_cb.pairing_state),
                    reason, bd_addr[0], bd_addr[1], bd_addr[2], bd_addr[3], bd_addr[4], bd_addr[5],
                    p_dev_rec->sec_bd_name);
...
void btm_sec_link_key_notification(uint8_t *p_bda, uint8_t *p_link_key, uint8_t key_type)
{
    ...
    if((!(p_dev_rec->sec_flags & BTM_SEC_NAME_KNOWN)
        ...
        BTM_TRACE_EVENT("rmt_io_caps:%d, sec_flags:x%x, dev_class[1]:x%02x", p_dev_rec->rmt_io_caps,
                        p_dev_rec->sec_flags, p_dev_rec->dev_class[1])
        return;
    }
        ...
        case BTM_PAIR_STATE_WAIT_DISCONNECT:
            ...
            if(p_dev_rec == NULL) {
                BTM_TRACE_ERROR("%s BTM_PAIR_STATE_WAIT_DISCONNECT unknown BDA: %08x%04x",
                                __func__,
                                (p_cb->pairing_bda[0] << 24) + (p_cb->pairing_bda[1] << 16) + (p_cb->pairing_bda[2] << 8) +
                                p_cb->pairing_bda[3],
        ...
        if(p_dev_rec->sec_flags & BTM_SEC_NAME_KNOWN) {
            ...
            if(p_cb->api.p_pin_callback) {
                (*p_cb->api.p_pin_callback)(p_bda, p_dev_rec->dev_class, p_dev_rec->sec_bd_name,
                                            (p_dev_rec->p_cur_service == NULL) ? FALSE
                                            : (uint8_t)(p_dev_rec->p_cur_service->security_flags
                                               & BTM_SEC_IN_MIN_16_DIGIT_PIN));
            }
        } else {
...
static tBTM_STATUS btm_sec_execute_procedure(tBTM_SEC_DEV_REC *p_dev_rec)
{
    ...
    BTM_TRACE_EVENT("Security Manager: trusted:0x%04x%04x", p_dev_rec->trusted_mask[1],
                    p_dev_rec->trusted_mask[0]);
...
static uint8_t btm_sec_check_prefetch_pin(tBTM_SEC_DEV_REC  *p_dev_rec)
{
    ...
    if((major == BTM_COD_MAJOR_AUDIO)
        ...
        if(btm_cb.pin_code_len) {
            BTM_PINCodeReply(p_dev_rec->bd_addr, BTM_SUCCESS, btm_cb.pin_code_len, btm_cb.pin_code,
                             p_dev_rec->trusted_mask);
        } else {
