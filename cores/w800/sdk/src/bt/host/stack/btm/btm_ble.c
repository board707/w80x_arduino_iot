...
**
*******************************************************************************/
tBTM_SEC_ACTION btm_ble_determine_security_act(uint8_t is_originator, BD_ADDR bdaddr,
                         ...
                         uint8_t pass_to_application)
{
    ...
    uint8_t i;
    BTM_TRACE_DEBUG("btm_sec_save_le_key key_type=0x%x pass_to_application=%d", key_type,
                    pass_to_application);
    ...
    BTM_TRACE_WARNING("BLE key type 0x%02x called for Unknown BDA or type: %08x%04x !! (btm_sec_save_le_key)",
                      key_type,
                      (bd_addr[0] << 24) + (bd_addr[1] << 16) + (bd_addr[2] << 8) + bd_addr[3],
                      (bd_addr[4] << 8) + bd_addr[5]);
        ...
        switch(event) {
            ...
            case SMP_COMPLT_EVT:
                ...
                if(event == SMP_COMPLT_EVT) {
                    BTM_TRACE_DEBUG("evt=SMP_COMPLT_EVT before update sec_level=0x%x sec_flags=0x%x",
