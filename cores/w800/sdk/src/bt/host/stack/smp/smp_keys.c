...
#if SMP_DEBUG == TRUE && SMP_DEBUG_VERBOSE == TRUE
    smp_debug_print_nbyte_little_endian(p_out->param_buf, (const uint8_t *)"Encrypted text",
                                        SMP_ENCRYT_KEY_SIZE);
#endif
...
**
*******************************************************************************/
uint8_t smp_calculate_f6(uint8_t *w, uint8_t *n1, uint8_t *n2, uint8_t *r, uint8_t *iocap,
                         uint8_t *a1,
...
uint8_t smp_calculate_link_key_from_long_term_key(tSMP_CB *p_cb)
{
    ...
    if(!ret) {
        ...
        } else if(HCI_SC_CTRLR_SUPPORTED(btm_cb.devcb.local_lmp_features[HCI_EXT_FEATURES_PAGE_2]))
