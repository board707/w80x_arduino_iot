...
**
*******************************************************************************/
void rfc_send_pn(tRFC_MCB *p_mcb, uint8_t dlci, uint8_t is_command, uint16_t mtu, uint8_t cl,
                 uint8_t k)
{
    uint8_t    *p_data;
    BT_HDR   *p_buf = (BT_HDR *)GKI_getbuf(RFCOMM_CMD_BUF_SIZE);
    ...
    switch(p_rx_frame->type) {
        ...
        case RFCOMM_MX_RPN:
            ...
            if(!p_rx_frame->u.rpn.is_request) {
                ...
                p_rx_frame->u.rpn.parity_type = (*p_data++ >> RFCOMM_RPN_PARITY_TYPE_SHIFT) &
