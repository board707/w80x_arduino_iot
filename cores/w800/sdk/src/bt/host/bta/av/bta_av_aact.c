...
**
*******************************************************************************/
static void bta_av_proc_stream_evt(uint8_t handle, BD_ADDR bd_addr, uint8_t event,
            ...
            switch(event) {
                case AVDT_RECONFIG_CFM_EVT:
                    APPL_TRACE_DEBUG("reconfig cfm event codec info = 0x%06x-%06x-%06x-%02x",
                                     (p_msg->msg.reconfig_cfm.p_cfg->codec_info[0] << 16) + (p_msg->msg.reconfig_cfm.p_cfg->codec_info[1]
                                             << 8) + p_msg->msg.reconfig_cfm.p_cfg->codec_info[2],
                                     (p_msg->msg.reconfig_cfm.p_cfg->codec_info[3] << 16) + (p_msg->msg.reconfig_cfm.p_cfg->codec_info[4]
                                             << 8) + p_msg->msg.reconfig_cfm.p_cfg->codec_info[5],
                                     (p_msg->msg.reconfig_cfm.p_cfg->codec_info[6] << 16) + (p_msg->msg.reconfig_cfm.p_cfg->codec_info[7]
                                             << 8) + p_msg->msg.reconfig_cfm.p_cfg->codec_info[8],
...
void bta_av_stream_data_cback(uint8_t handle, BT_HDR *p_pkt, uint32_t time_stamp, uint8_t m_pt)
{
    int index = 0;
    tBTA_AV_SCB         *p_scb ;
    APPL_TRACE_DEBUG("bta_av_stream_data_cback avdt_handle: %d pkt_len=0x%x  ofst = 0x%x", handle,
...
void bta_av_role_res(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_av_role_res q_tag:%d, wait:x%x, role:x%x", p_scb->q_tag, p_scb->wait,
                     p_scb->role);
...
void bta_av_config_ind(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    if((p_evt_cfg->num_codec == 0) ||
        ...
        APPL_TRACE_DEBUG("bta_av_config_ind: SEID: %d use_rc: %d cur_psc_mask:0x%x", p_info->seid,
...
void bta_av_setconfig_rsp(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_av_setconfig_rsp: sep_idx: %d cur_psc_mask:0x%x", p_scb->sep_idx,
                     p_scb->cur_psc_mask);
...
void bta_av_do_start(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    uint8_t       cur_role;
    APPL_TRACE_DEBUG("bta_av_do_start sco_occupied:%d, role:x%x, started:%d", bta_av_cb.sco_occupied,
...
void bta_av_start_failed(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    bta_sys_set_policy(BTA_ID_AV, (HCI_ENABLE_SNIFF_MODE | HCI_ENABLE_MASTER_SLAVE_SWITCH),
                       p_scb->peer_addr);
    p_scb->sco_suspend = FALSE;
}
...
void bta_av_rcfg_cfm(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    */
    if(err_code) {
        ...
        wm_memcpy(p_scb->cfg.codec_info, p_data->str_msg.msg.reconfig_cfm.p_cfg->codec_info,
                  AVDT_CODEC_SIZE);
...
{
    tBTA_AV_START   start;
    APPL_TRACE_DEBUG("bta_av_open_rc use_rc: %d, wait: x%x role:x%x", p_scb->use_rc, p_scb->wait,
                     p_scb->role);
...
void bta_av_offload_req(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    if(p_scb->started != TRUE) {
        ...
        if(L2CA_GetConnectionConfig(p_scb->l2c_cid, &a2dp_offload_start.acl_data_size,
            ...
            a2dp_offload_start.stream_source = ((uint32_t)(p_scb->cfg.codec_info[1] |
                                                p_scb->cfg.codec_info[2]));
