...
static void bta_av_sys_rs_cback(tBTA_SYS_CONN_STATUS status, uint8_t id, uint8_t app_id,
                                BD_ADDR peer_addr);
                ...
                if(AVDT_CreateStream(&p_scb->seps[index].av_handle, &cs) == AVDT_SUCCESS) {
                    ...
                    if(cs.tsep == AVDT_TSEP_SNK) {
                        ...
                        p_scb->seps[index].p_app_data_cback =
                                        NULL;    /* In case of A2DP SOURCE we don't need a callback to handle media packets */
                    }
                }
            }
