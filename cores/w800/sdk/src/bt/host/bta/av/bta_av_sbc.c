...
**
*******************************************************************************/
void bta_av_sbc_init_up_sample(uint32_t src_sps, uint32_t dst_sps, uint16_t bits,
                               uint16_t n_channels)
{
...
uint8_t bta_av_sbc_cfg_for_cap(uint8_t *p_peer, tA2D_SBC_CIE *p_cap, tA2D_SBC_CIE *p_pref)
{
    ...
    if(peer_cie.num_subbands & p_pref->num_subbands) {
        ...
        APPL_TRACE_ERROR("bta_av_sbc_cfg_for_cap: num_subbands(0x%02X) not supported",
                         p_pref->num_subbands);
        return A2D_FAIL;
    }
...
uint8_t bta_av_sbc_cfg_matches_cap(uint8_t *p_cfg, tA2D_SBC_CIE *p_cap)
{
    ...
    APPL_TRACE_DEBUG(" MAX_BitPool peer: 0%x, capability  0%x", cfg_cie.max_bitpool,
                     p_cap->max_bitpool);
    APPL_TRACE_DEBUG(" Min_bitpool peer: 0%x, capability  0%x", cfg_cie.min_bitpool,
                     p_cap->min_bitpool);
