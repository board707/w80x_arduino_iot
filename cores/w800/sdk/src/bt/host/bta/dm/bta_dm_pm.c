...
static void bta_dm_pm_cback(tBTA_SYS_CONN_STATUS status, uint8_t id, uint8_t app_id,
                            BD_ADDR peer_addr);
...
static void bta_dm_pm_btm_cback(BD_ADDR bd_addr, tBTM_PM_STATUS status, uint16_t value,
                                uint8_t hci_status);
                            ...
                            BD_ADDR peer_addr)
{
    ...
    if(p_bta_dm_pm_spec[p_bta_dm_pm_cfg[i].spec_idx].actn_tbl[status][0].power_mode ==
            BTA_DM_PM_NO_ACTION) {
#if (BTM_SSR_INCLUDED == TRUE)
    ...
    if(p_bta_dm_pm_spec[p_bta_dm_pm_cfg[i].spec_idx].actn_tbl[status][0].power_mode ==
        ...
        if(j != bta_dm_conn_srvcs.count) {
            ...
            for(; j < bta_dm_conn_srvcs.count ; j++) {
                wm_memcpy(&bta_dm_conn_srvcs.conn_srvc[j], &bta_dm_conn_srvcs.conn_srvc[j + 1],
                               ...
                               tBTA_DM_PM_REQ pm_req)
{
    ...
    for(i = 0; i < bta_dm_conn_srvcs.count ; i++) {
        ...
        if(!bdcmp(p_srvcs->peer_bdaddr, peer_addr)) {
            ...
            APPL_TRACE_DEBUG("bta_dm_pm_set_mode: srvcsid: %d, state: %d, j: %d", p_srvcs->id, p_srvcs->state,
                             j);
...
static void bta_dm_pm_hid_check(uint8_t bScoActive)
{
    ...
    for(j = 0; j < bta_dm_conn_srvcs.count ; j++) {
        ...
        if(bta_dm_conn_srvcs.conn_srvc[j].id == BTA_ID_HH) {
            ...
            bta_dm_pm_set_sniff_policy(bta_dm_find_peer_device(bta_dm_conn_srvcs.conn_srvc[j].peer_bdaddr),
                                       bScoActive);
