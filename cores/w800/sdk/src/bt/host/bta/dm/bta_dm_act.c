...
#include "../include/bta_dm_co.h"
#include "../include/bta_dm_int.h"
#include "../include/bta_sys.h"
#include "../../stack/include/btm_api.h"
#include "../../stack/include/btm_int.h"
#include "../../stack/include/btu.h"
#include "../../stack/include/gap_api.h"    /* For GAP_BleReadPeerPrefConnParams */
...
static void bta_dm_policy_cback(tBTA_SYS_CONN_STATUS status, uint8_t id, uint8_t app_id,
                                BD_ADDR peer_addr);
...
static void bta_dm_rm_cback(tBTA_SYS_CONN_STATUS status, uint8_t id, uint8_t app_id,
                            BD_ADDR peer_addr);
...
void bta_dm_add_device(tBTA_DM_MSG *p_data)
{
    ...
    if(p_dev->is_trusted) {
        ...
        while(p_dev->tm && (index < BTA_MAX_SERVICE_ID)) {
            if(p_dev->tm & (uint32_t)(1 << index)) {
                ...
                trusted_services_mask[btm_mask_index] |= (uint32_t)(1 <<
                        (bta_service_id_to_btm_srv_id_lkup_tbl[index] - (uint32_t)(btm_mask_index * 32)));
    ...
    if(!BTM_SecAddDevice(p_dev->bd_addr, p_dc, p_dev->bd_name, p_dev->features,
        ...
        APPL_TRACE_ERROR("BTA_DM: Error adding device %08x%04x",
                         (p_dev->bd_addr[0] << 24) + (p_dev->bd_addr[1] << 16) + (p_dev->bd_addr[2] << 8) +
                         p_dev->bd_addr[3],
...
void bta_dm_pin_reply(tBTA_DM_MSG *p_data)
{
    ...
    if(p_data->pin_reply.accept) {
        BTM_PINCodeReply(p_data->pin_reply.bd_addr, BTM_SUCCESS, p_data->pin_reply.pin_len,
                         p_data->pin_reply.p_pin, trusted_mask);
    } else {
...
static uint8_t bta_dm_read_remote_device_name(BD_ADDR bd_addr, tBT_TRANSPORT transport)
{
    ...
    if(btm_status == BTM_CMD_STARTED) {
        ...
        APPL_TRACE_WARNING("bta_dm_read_remote_device_name: BTM_ReadRemoteDeviceName returns 0x%02X",
                           btm_status);
        return (FALSE);
    }
}
...
void bta_dm_disc_result(tBTA_DM_MSG *p_data)
{
    ...
    if((bta_dm_search_cb.services & ((BTA_ALL_SERVICE_MASK | BTA_USER_SERVICE_MASK) &
...
static void bta_dm_service_search_remname_cback(BD_ADDR bd_addr, DEV_CLASS dc, BD_NAME bd_name)
{
    ...
    if(!bdcmp(bta_dm_search_cb.peer_bdaddr, bd_addr)) {
        ...
        if(btm_status == BTM_BUSY) {
            ...
            APPL_TRACE_WARNING("bta_dm_service_search_remname_cback: BTM_ReadRemoteDeviceName returns 0x%02X",
                               btm_status);
            rem_name.length = 0;
    ...
    if(bta_dm_cb.cur_av_count) {
        for(i = 0; i < bta_dm_cb.device_list.count; i++) {
            ...
            if((p_dev->conn_state == BTA_DM_CONNECTED) && (p_dev->info & BTA_DM_DI_AV_ACTIVE) &&
                ...
                if(BTM_CMD_STARTED == BTM_SwitchRole(p_dev->peer_bdaddr, HCI_ROLE_MASTER,
    ...
    switch(p_data->acl_change.event) {
        ...
        case BTM_BL_ROLE_CHG_EVT:   /* role change event */
            ...
            if(p_dev) {
                ...
                bta_sys_notify_role_chg(p_data->acl_change.bd_addr, p_data->acl_change.new_role,
    ...
    if(is_new) {
        ...
        for(i = 0; i < bta_dm_cb.device_list.count; i++) {
            ...
            for(; i < bta_dm_cb.device_list.count ; i++) {
                wm_memcpy(&bta_dm_cb.device_list.peer_device[i], &bta_dm_cb.device_list.peer_device[i + 1],
...
**
*******************************************************************************/
void bta_dm_encrypt_cback(BD_ADDR bd_addr, tBT_TRANSPORT transport, void *p_ref_data,
                          tBTM_STATUS result)
{
...
static void bta_dm_gatt_disc_result(tBTA_GATT_ID service_id)
{
    ...
    if(bta_dm_search_cb.ble_raw_used + sizeof(tBTA_GATT_ID) < bta_dm_search_cb.ble_raw_size) {
        ...
        APPL_TRACE_ERROR("%s out of room to accomodate more service ids ble_raw_size = %d ble_raw_used = %d",
                         __FUNCTION__, bta_dm_search_cb.ble_raw_size, bta_dm_search_cb.ble_raw_used);
    }
