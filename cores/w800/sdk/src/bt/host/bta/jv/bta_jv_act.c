...
void bta_jv_rfcomm_connect(tBTA_JV_MSG *p_data)
{
    ...
    */
    ...
    if(0 == sec_id ||
        ...
        evt_data.status = BTA_JV_FAILURE;
        APPL_TRACE_ERROR("sec_id:%d is zero or BTM_SetSecurityLevel failed, remote_scn:%d", sec_id,
                         cc->remote_scn);
    }
    ...
    if(p_cb->max_sess > 1) {
        ...
        if(used < p_cb->max_sess && listen == 1 && si) {
            ...
            if(RFCOMM_CreateConnection(p_cb->sec_id, p_cb->scn, TRUE,
                                       BTA_JV_DEF_RFC_MTU, (uint8_t *) bd_addr_any, &(p_cb->rfc_hdl[si]),
...
static void fcchan_conn_chng_cbk(uint16_t chan, BD_ADDR bd_addr, uint8_t connected, uint16_t reason,
