...
void bt_mesh_model_foreach(void (*func)(struct bt_mesh_model *mod, struct bt_mesh_elem *elem,
                                        bool vnd, bool primary, void *user_data), void *user_data)
{
    int i, j;
                       ...
                       bool vnd, bool primary, void *user_data)
{
    ...
    if(mod->pub) {
        mod->pub->mod = NULL;
        k_delayed_work_deinit(&mod->pub->timer);
    }
    ...
    if(mod->cb && mod->cb->deinit) {
        ...
        if(err != 0) {
            BT_ERR("mod cb deinit err=%d", err);
        }
    }
}
...
void bt_mesh_model_recv(struct bt_mesh_net_rx *rx, struct os_mbuf *buf)
{
    ...
    for(i = 0; i < dev_comp->elem_count; i++) {
        ...
        {
            BT_DBG("Found OpCode 0x%08x for elem %d", opcode, i);
        }

        if(!model_has_key(model, rx->ctx.app_idx)) {
            BT_DBG("No key OpCode 0x%08x for elem %d", opcode, i);
            continue;
        }

        if(!model_has_dst(model, rx->ctx.recv_dst)) {
            BT_DBG("No dst OpCode 0x%08x for elem %d", opcode, i);
            continue;
        }
                      ...
                      const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    BT_DBG(">>>@>>>model=0x%p, app_idx=0x%04x\r\n", model, tx->ctx->app_idx);
...
int bt_mesh_model_publish(struct bt_mesh_model *model)
{
    ...
    BT_DBG("Publish TTL=%u Retransmit Count %u Interval %ums\r\n", pub->ttl, pub->count,
