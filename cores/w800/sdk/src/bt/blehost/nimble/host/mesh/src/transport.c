...
static void update_rpl(struct bt_mesh_rpl *rpl, struct bt_mesh_net_rx *rx)
{
    ...
    if(IS_ENABLED(CONFIG_BT_SETTINGS) && IS_ENABLED(CONFIG_BT_SETTINGS_RPL)) {
                           ...
                           struct os_mbuf *buf)
{
    ...
    if(rx->ctx.recv_dst != hb_sub_dst) {
        BT_WARN("Ignoring heartbeat to non-subscribed destination[0x%04x,0x%04x]", rx->ctx.recv_dst, hb_sub_dst);
        return 0;
    }
    ...
    BT_DBG("src 0x%04x dst 0x%04x TTL %u InitTTL %u (%u hop%s) feat 0x%04x",
           rx->ctx.addr, rx->ctx.recv_dst, rx->ctx.recv_ttl, init_ttl, hops,
           (hops == 1) ? "" : "s", feat);
                       ...
                       u64_t *seq_auth)
{
    ...
    if(is_replay(rx, NULL)) {
        BT_INFO("Replay: src 0x%04x dst 0x%04x seq 0x%06x",
...
void bt_mesh_trans_deinit(void)
{
    ...
    for(i = 0; i < ARRAY_SIZE(seg_tx); i++) {
        k_delayed_work_deinit(&seg_tx[i].retransmit);
    }
     ...
     */
    for(i = 0; i < ARRAY_SIZE(seg_rx); i++) {
        if(seg_rx[i].buf) { os_mbuf_free_chain(seg_rx[i].buf); }

        k_delayed_work_deinit(&seg_rx[i].ack);
    }
}
...
void bt_mesh_heartbeat_send_ext(u16_t src_addr)
{
