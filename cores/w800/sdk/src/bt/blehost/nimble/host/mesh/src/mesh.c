...
#include "../include/host/ble_uuid.h"
...
bool bt_mesh_role_is_provisioner(void)
{
    return atomic_test_bit(bt_mesh.flags, BT_MESH_PROVISIONER);
}
bool bt_mesh_role_is_node(void)
{
    return atomic_test_bit(bt_mesh.flags, BT_MESH_NODE);
}

int bt_mesh_get_role(void)
{
    if(atomic_test_bit(bt_mesh.flags, BT_MESH_NODE)) {
        return (int)1;
    } else if(atomic_test_bit(bt_mesh.flags, BT_MESH_PROVISIONER)) {
        return (int)2;
    } else {
        return 0; /*MESH_ROLE_UNKNOWN*/
    }
}
...
int bt_mesh_deinit(void)
{
    ...
    BT_DBG("bt_mesh_net_deinit");
    bt_mesh_net_deinit();
    BT_DBG("bt_mesh_trans_deinit");
    bt_mesh_trans_deinit();
    BT_DBG("bt_mesh_beacon_deinit");
    bt_mesh_beacon_deinit();
    bt_mesh_adv_deinit();
    bt_mesh_settings_deinit();
    BT_DBG("bt_mesh_proxy_deinit");
#if (MYNEWT_VAL(BLE_MESH_PROXY))
    bt_mesh_proxy_deinit();
#endif
#if (MYNEWT_VAL(BLE_MESH_PROV))
    BT_DBG("bt_mesh_prov_deinit");
    err = bt_mesh_prov_deinit();

    if(err) {
        BT_WARN("bt_mesh_prov_deinit failed");
    }
    ...
    if(err) {
        BT_WARN("ble_gatts_reset failed");
    }

#endif
    BT_DBG("bt_mesh_comp_unregister");
    err = bt_mesh_comp_unregister();

    if(err) {
        BT_WARN("bt_mesh_comp_unregister failed");
    }

#if (MYNEWT_VAL(BLE_MESH_PROV))
    /* Need this to proper link.rx.buf allocation */
    bt_mesh_prov_reset_link_deinit();
#endif
    memset(bt_mesh.dev_key, 0, sizeof(bt_mesh.dev_key));
    ble_gap_event_listener_unregister(&mesh_event_listener);

    return err;
}
void bt_mesh_role_set(bool node_role)
{
    if(node_role) {
...
int bt_mesh_provision_init(void)
{
    ...
    if(bt_rand(net_key, 16)) {
        BT_WARN("Provisioner generate primary net key failed");
        return -1;
    }

    if(bt_mesh_net_keys_create(&sub->keys[0], net_key)) {
        BT_ERR("Provisioner create net-related keys failed");
        return -EIO;
    }
