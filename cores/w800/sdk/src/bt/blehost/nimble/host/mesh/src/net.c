...
u32_t bt_mesh_next_seq(void)
{
    ...
    if(IS_ENABLED(CONFIG_BT_SETTINGS) && IS_ENABLED(CONFIG_BT_SETTINGS_SEQ)) {
        bt_mesh_store_seq();
    }
...
{
    int err;
    BT_DBG("src 0x%04x dst 0x%04x ttl 0x%02x len %u headroom %zu tailroom %zu",
           tx->src, tx->ctx->addr, tx->ctx->send_ttl, buf->om_len, net_buf_headroom(buf),
           net_buf_tailroom(buf));
    BT_DBG("Payload len %u: %s", buf->om_len, bt_hex(buf->om_data, buf->om_len));
    BT_DBG("Seq 0x%06x", bt_mesh.seq);

    if(tx->ctx->send_ttl == BT_MESH_TTL_DEFAULT) {
        tx->ctx->send_ttl = min(bt_mesh_default_ttl_get(), bt_mesh_dist_map_get(tx->ctx->addr) +1);
        BT_DBG("bt_mesh_net_send dst 0x%04x TTL 0x%02x, [%02x,%02x]", tx->ctx->addr, tx->ctx->send_ttl, bt_mesh_default_ttl_get(), bt_mesh_dist_map_get(tx->ctx->addr));
    }
                       ...
                       u32_t iv_index, const u8_t auth[8])
{
    ...
    if(memcmp(net_id, keys->net_id, 8)) {
        BT_INFO("!!!net_id %s != %s",
                bt_hex(net_id, 8), bt_hex(keys->net_id, 8));
        return false;
    }
                       ...
                       struct os_mbuf *buf)
{
    ...
    if(rx->net_if == BT_MESH_NET_IF_ADV && msg_cache_match(rx, buf)) {
        BT_INFO("Duplicate found in Network Message Cache");
        return -EALREADY;
    }
                              ...
                              struct bt_mesh_net_rx *rx)
{
    ...
    BT_DBG("Relay, TTL %u CTL %u dst 0x%04x\r\n", rx->ctx.recv_ttl, rx->ctl,
           rx->ctx.recv_dst);
    ...
    if(bt_mesh_adv_avaiable_for_relay() == 0)
    {
        BT_WARN("Hungray of relay buffers");
        return;
    }
    ...
    if(rx->net_if != BT_MESH_NET_IF_LOCAL) {
        ...
        sbuf->om_data[1] &= 0x80;
        sbuf->om_data[1] |= min(rx->ctx.recv_ttl - 1, bt_mesh_dist_map_get(rx->ctx.recv_dst));
        BT_DBG("Relay dst 0x%04x get min ttl:%u,%u\r\n", rx->ctx.recv_dst , rx->ctx.recv_ttl - 1, bt_mesh_dist_map_get(rx->ctx.recv_dst));
    }
    ...
    BT_DBG("Relaying packet. TTL is now %u \r\n", TTL(buf->om_data));
    ...
    if(relay_to_adv(rx->net_if)) {
        bt_mesh_adv_send_to_tail(buf, NULL, NULL);
    }
                       ...
                       struct bt_mesh_net_rx *rx, struct os_mbuf *buf)
{
    ...
    if(net_if == BT_MESH_NET_IF_ADV && check_dup(data)) {
        BT_INFO("duplicate packet; dropping %u bytes: %s", data->om_len,
                      ...
                      enum bt_mesh_net_if net_if)
{
    ...
    if(!bt_mesh_is_provisioned()) {
        BT_INFO("Not provisioned; dropping packet");
        goto done;
    }
...
void bt_mesh_net_deinit(void)
{
    ...
    /*Flush all cached msg*/
    while((buf = net_buf_slist_get(&bt_mesh.local_queue))) {
        net_buf_unref(buf);
    }

    k_delayed_work_deinit(&bt_mesh.ivu_timer);
    k_work_deinit(&bt_mesh.local_work);
}
#endif
