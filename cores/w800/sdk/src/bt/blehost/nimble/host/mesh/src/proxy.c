...
#if (MYNEWT_VAL(BLE_MESH_PROXY) && MYNEWT_VAL(BLE_MESH))
...
#define PROXY_CONN_PARAM_UPDATE_TIMEOUT K_MSEC(50)
...
static const struct ble_gap_adv_params slow_adv_param = {
    ...
    .itvl_min = 0xA0,//BT_GAP_ADV_SLOW_INT_MIN,
    .itvl_max = 0xF0,//BT_GAP_ADV_SLOW_INT_MAX,
};
...
static struct bt_mesh_proxy_client {
    ...
    struct k_delayed_work conn_param_update_timer;
...
static void proxy_complete_pdu(struct bt_mesh_proxy_client *client)
{
    switch(client->msg_type) {
        ...
        case BT_MESH_PROXY_BEACON:
            ...
            bt_mesh_beacon_recv(NULL, client->buf);
            break;
...
static void proxy_param_update_cb(uint16_t conn_handle, int status, void *arg)
{
    BT_INFO("conn param update complete; conn_handle=%d status=%d\n",
                            conn_handle, status);
}

static void proxy_conn_param_update_timeout(struct ble_npl_event *work)
{
    struct ble_l2cap_sig_update_params params;
    struct bt_mesh_proxy_client *client;
    int rc;
    BT_WARN("Proxy CPU timeout");
    client = ble_npl_event_get_arg(work);
    assert(client != NULL);

    if((client->conn_handle != BLE_HS_CONN_HANDLE_NONE)) {
        params.itvl_min = 0x0016;
        params.itvl_max = 0x0026;
        params.slave_latency = 0;
        params.timeout_multiplier = 500;
        rc = ble_l2cap_sig_update(client->conn_handle, &params,
                                  proxy_param_update_cb, NULL);
...
static void proxy_connected(uint16_t conn_handle)
{
    ...
    memset(client->filter, 0, sizeof(client->filter));
    net_buf_simple_init(client->buf, 0);
    //k_delayed_work_submit(&client->conn_param_update_timer, PROXY_CONN_PARAM_UPDATE_TIMEOUT);
}
...
static void proxy_disconnected(uint16_t conn_handle, int reason)
{
    ...
    if(disconnected) {
        BT_WARN("----------------->conn_handle %d reason %d(0x%04x)[0x%02x]", conn_handle, reason, reason,
                reason - 0x200);
        bt_mesh_adv_update();
    }
}
...
#if (MYNEWT_VAL(BLE_MESH_PB_GATT))
static void prov_ccc_write(uint16_t conn_handle, uint8_t on)
{
    struct bt_mesh_proxy_client *client;
...
#if (MYNEWT_VAL(BLE_MESH_PB_GATT))
            prov_ccc_write(event->subscribe.conn_handle, event->subscribe.cur_indicate|event->subscribe.cur_notify);
#endif
        }
    }
...
int bt_mesh_proxy_init(void)
{
    ...
    for(i = 0; i < MYNEWT_VAL(BLE_MAX_CONNECTIONS); ++i) {
        ...
        k_delayed_work_init(&clients[i].conn_param_update_timer, proxy_conn_param_update_timeout);
        k_delayed_work_add_arg(&clients[i].conn_param_update_timer, &clients[i]);        
    }
...
int bt_mesh_proxy_deinit(void)
{
    ...
    for(i = 0; i < MYNEWT_VAL(BLE_MAX_CONNECTIONS); ++i) {
#if (MYNEWT_VAL(BLE_MESH_GATT_PROXY))
        k_work_deinit(&clients[i].send_beacons);
#endif
        ...
        clients[i].conn_handle = BLE_HS_CONN_HANDLE_NONE;
        k_delayed_work_deinit(&clients[i].sar_timer);
        k_delayed_work_deinit(&clients[i].conn_param_update_timer);
    }
