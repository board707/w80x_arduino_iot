...
static void unprovisioned_beacon_recv(const bt_addr_le_t *addr, struct os_mbuf *buf)
{
    ...
    if(prov->unprovisioned_beacon) {
        prov->unprovisioned_beacon(addr, uuid,
...
{
    /* Don't send anything if we have an active proxy connection */
    if((MYNEWT_VAL(BLE_MESH_PROV)) && !bt_mesh_proxy_not_connected()) {
...
void bt_mesh_beacon_recv(const bt_addr_le_t *addr, struct os_mbuf *buf)
{
    ...
    switch(type) {
        case BEACON_TYPE_UNPROVISIONED:
            unprovisioned_beacon_recv(addr, buf);
            break;
...
{
    k_delayed_work_deinit(&beacon_timer);
}
