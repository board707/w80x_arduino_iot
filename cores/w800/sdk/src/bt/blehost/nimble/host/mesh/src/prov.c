...
#if (MYNEWT_VAL(BLE_MESH_PROV) && MYNEWT_VAL(BLE_MESH))
...
static void reset_adv_link(bool prov_success)
{
    prov_clear_tx();

    if(prov->link_close) {
        prov->link_close(BT_MESH_PROV_ADV, prov_success);
    }
...
static void prov_retransmit(struct ble_npl_event *work)
{
    ...
    if(k_uptime_get() - link.tx.start > timeout) {
        ...
        reset_adv_link((timeout == CLOSING_TIMEOUT));
        return;
    }
...
int bt_mesh_pb_gatt_close(uint16_t conn_handle)
{
    ...
    if(prov->link_close) {
        prov->link_close(BT_MESH_PROV_GATT, true);
    }
...
int bt_mesh_prov_deinit(void)
{
    ...
    BT_DBG("");
    k_delayed_work_deinit(&link.prot_timer);
    prov = NULL;
#if MYNEWT_VAL(BLE_MESH_PB_ADV)
    k_delayed_work_deinit(&link.tx.retransmit);
#endif
...
void bt_mesh_prov_reset_link_deinit(void)
{
    ...
    if(link.rx.buf) {
        os_mbuf_free_chain(link.rx.buf);
    }
