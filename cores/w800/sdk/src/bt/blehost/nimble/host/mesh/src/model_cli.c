...
{
    BT_DBG("!!!!!!!!onoff_cli_init model=0x%p\r\n", model);
    ...
    if(gen_onoff_cli->init) gen_onoff_cli->init(model);
    k_sem_init(&gen_onoff_cli->op_sync, 0, 1);
    return 0;
}
static int onoff_cli_deinit(struct bt_mesh_model *model)
{
    if(!gen_onoff_cli) { return -EALREADY; }

    BT_DBG("");
    if(gen_onoff_cli->deinit) gen_onoff_cli->deinit(model);
    k_sem_deinit(&gen_onoff_cli->op_sync);
    gen_onoff_cli = NULL;
    return 0;
}
...
{
    if(!gen_level_cli) { return -EALREADY; }

    BT_DBG("");
    k_sem_deinit(&gen_level_cli->op_sync);
    gen_level_cli = NULL;
