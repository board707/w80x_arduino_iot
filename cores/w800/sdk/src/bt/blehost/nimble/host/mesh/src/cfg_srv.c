...
#include "../../../../porting/w800/include/syscfg/syscfg.h"
                        ...
                        struct os_mbuf *buf)
{
    ...
    BT_DBG("vvvvvvvvvvvvvvvv model=%p AppIdx 0x%04x NetIdx 0x%04x", model, key_app_idx, key_net_idx);
                           ...
                           struct os_mbuf *buf)
{
    ...
    u8_t status;
    key_idx_unpack(buf, &key_net_idx, &key_app_idx);
    BT_DBG("xxxxxxxxxxxxxxxxxxxx AppIdx 0x%04x NetIdx 0x%04x", key_app_idx, key_net_idx);
    bt_mesh_model_msg_init(msg, OP_APP_KEY_STATUS);
...
int bt_mesh_app_key_add(u16_t net_idx, u16_t app_idx, const u8_t val[16],
                        bool update)
{
    ...
    app->app_idx = app_idx;
    app->updated = update;
    memcpy(app->keys[0].val, val, 16);
    memcpy(app->keys[1].val, val, 16);
    bt_mesh_app_id(app->keys[0].val, &app->keys[0].id);
    bt_mesh_app_id(app->keys[1].val, &app->keys[1].id);
    BT_DBG("AppKeyAdded net_idx 0x%03x app_idx 0x%03x", net_idx, app_idx);
...
send_status:
    BT_DBG("XXX mod pub set, status=%d\r\n", status);
    send_mod_pub_status(model, ctx, elem_addr, pub_addr, vnd, mod,
                        status, mod_id);
}
                              ...
                              struct os_mbuf *buf)
{
    ...
    BT_DBG("period_ms %u dst 0x%02x", (unsigned) period_ms, cfg->hb_sub.dst);
...
#define ALL_ELEM_HEARTBEAT_ENABLE
    if(!sub) {
...
#ifdef ALL_ELEM_HEARTBEAT_ENABLE    
    const struct bt_mesh_comp *dev_comp = bt_mesh_comp_get();

    for(size_t index = 0; index < dev_comp->elem_count; index++) {
        struct bt_mesh_elem *elem = &dev_comp->elem[index];
        bt_mesh_heartbeat_send_ext(elem->addr);
    }
#else
    bt_mesh_heartbeat_send();    
#endif
...
{
    if(!conf) { return -EALREADY; }

    struct bt_mesh_cfg_srv *cfg = model->user_data;

    BT_DBG("");

    k_delayed_work_deinit(&cfg->hb_pub.timer);
...
static void conf_dist_map_update(u16_t src, uint8_t min_hop, uint8_t max_hop)
{
    struct bt_mesh_cfg_srv *cfg = conf;
    u16_t priv_src;
#define MAX_DIST_MAP_TABLE_SIZE 0xFF

    for(uint8_t i=0; i<MAX_DIST_MAP_TABLE_SIZE; i++)
    {
        priv_src = (u16_t)((cfg->dist_map[i]&0xFFFF0000)>>16);

        if(priv_src == src || priv_src == 0)
        {
            cfg->dist_map[i] = src<<16|min_hop<<8|max_hop;
            return;
        }
    }
}
...
#define CARE_SRC_ADDR 0
    ...
    if(src != cfg->hb_sub.src || dst != cfg->hb_sub.dst) {
        BT_WARN("No subscription for received heartbeat[%04x,%04x][%04x,%04x]", src, cfg->hb_sub.src, dst, cfg->hb_sub.dst);
        return;
    }
#else
    ...
    BT_DBG("src 0x%04x dst 0x%04x hops %u min %u max %u count %u", src,
           ...
           cfg->hb_sub.count);
    conf_dist_map_update(src, cfg->hb_sub.min_hops, cfg->hb_sub.max_hops);
...
u8_t bt_mesh_dist_map_get(u16_t src)
{
    struct bt_mesh_cfg_srv *cfg = conf;
    u16_t priv_src;
#define MAX_DIST_MAP_TABLE_SIZE 0xFF
    ...
    for(uint8_t i=0; i<MAX_DIST_MAP_TABLE_SIZE; i++)
    {
        ...
        if(priv_src == src)
        {
            return (u8_t)((cfg->dist_map[i]&0xFF00)>>8);
        }
    } 
    return BT_MESH_TTL_MAX;
}
